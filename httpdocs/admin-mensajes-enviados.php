...
if ($_POST['respondermsg']) {

	$consulta="INSERT INTO `nearyou`.`mail` (`id`, `fecha`, `from`, `to`, `asunto`, `msg`, `news`) 
		VALUES (NULL, CURRENT_TIMESTAMP, '".$_SESSION['usuario']."', '".$_POST['para']."', '".$_POST['asunto']."', '".$_POST['msg']."', '0');";
	...
	mysql_query($consulta, $conex);
		...
		$titulo    = $_POST['asunto'];
		$mensaje   = $_POST['msg'];
		$cabeceras = 'From: '.$_SESSION['usuario'].'' . "\r\n" .
			'Reply-To: '.$_SESSION['usuario'].'' . "\r\n" .
...
<div  class="divContenedor" style='padding: 20px;margin: 0px; border-top: 0px;'>
	...
	<div style='width:45%; float: left;'>
			...
			<label for="check2">  <i class="fa fa-inbox"></i> <i class="fa fa-share"></i> Mensajes Enviados</label>
			<div class="texto">
				...
				<div style='height: 300px; overflow:auto;'>		
						...
						<?
							...
							$consulta="Select mail.id,mail.fecha,mail.from,mail.asunto,mail.news,usuarios.subdominio 
										from mail,usuarios 
										where  (usuarios.usuario=mail.to) and   `from`='".$campo['usuario']."' order by news ASC, id DESC LIMIT 100;"; 
	...
	<div style='width:54%; float: right;'>
			...
			<label for="check1">  <i class="fa fa-file-text-o"></i>  Mensaje</label>
